...
describe("RemoveGroupMemberService", () => {
    ...
    beforeAll(() => {
        const { unit, unitRef } = TestBed.create(RemoveGroupMemberService).compile();
    ...
    it('should not remove group member if user is not authorized', async () => {
        ...
        authenticatedGroupMember.role().removePermission(authenticatedGroupMember.role().findPermission(GroupPermissionName.REMOVE_MEMBER));
