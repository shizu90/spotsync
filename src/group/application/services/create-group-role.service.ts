...
export class CreateGroupRoleService implements CreateGroupRoleUseCase 
{
    ...
    public async execute(command: CreateGroupRoleCommand): Promise<CreateGroupRoleDto> 
    {
        ...
        const hasPermission = authenticatedGroupMember.role().permissions().map((p) => p.name()).includes('create-role');

        if(!(hasPermission || authenticatedGroupMember.isCreator() || authenticatedGroupMember.role().name() === 'administrator')) {
            throw new UnauthorizedAccessError(`You don't have permissions to create role`);
        }
        ...
        const groupRole = GroupRole.create(
            ...
            command.name, 
            command.hexColor,
            await Promise.all(command.permissionIds.map(async (pid) => {
        ...
        return new CreateGroupRoleDto(
            ...
            groupRole.name(),
            groupRole.hexColor(),
            groupRole.permissions().map((p) => {return {id: p.id(), name: p.name()}}),
            groupRole.createdAt(),
            groupRole.updatedAt(),
