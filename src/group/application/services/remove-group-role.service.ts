...
export class RemoveGroupRoleService implements RemoveGroupRoleUseCase 
{
    ...
    public async execute(command: RemoveGroupRoleCommand): Promise<void> 
    {
        ...
        const hasPermission = authenticatedGroupMember.role().permissions().map((p) => p.name()).includes('delete-role');

        if(!(hasPermission || authenticatedGroupMember.isCreator() || authenticatedGroupMember.role().name() === 'administrator')) {
            throw new UnauthorizedAccessError(`You don't have permissions to delete role`);
        }
