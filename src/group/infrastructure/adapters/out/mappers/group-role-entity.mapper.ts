import { GroupPermission as GroupPermissionPrisma, GroupRolePermission as GroupRolePermissionPrisma, GroupRole as GroupRolePrisma } from "@prisma/client";
...
export type GroupRoleEntity = GroupRolePrisma & {permissions?: (GroupRolePermissionPrisma & {group_permission?: GroupPermissionPrisma})[], group?: GroupEntity};

export class GroupRoleEntityMapper implements EntityMapper<GroupRole, GroupRoleEntity> {
    ...
    public toEntity(model: GroupRole): GroupRoleEntity {
        ...
        return {
            ...
            permissions: model.permissions() ? model.permissions().map(permission => ({
    ...
    public toModel(entity: GroupRoleEntity): GroupRole {
        ...
        return GroupRole.create(
            ...
            entity.name,
            entity.hex_color,
            entity.permissions ? entity.permissions.map(permission => GroupPermission.create(
                permission.group_permission ? permission.group_permission.id : null,
                permission.group_permission ? permission.group_permission.name as GroupPermissionName : null,
            )) : null,
            entity.is_immutable,
