...
import {
	GroupRoleEntity,
	GroupRoleEntityMapper,
} from './group-role-entity.mapper';

export type GroupMemberEntity = GroupMemberPrisma & {
	group_role?: GroupRoleEntity;
	group?: GroupEntity;
	user?: UserEntity;
};

export class GroupMemberEntityMapper
	implements EntityMapper<GroupMember, GroupMemberEntity>
{
	...
	public toEntity(model: GroupMember): GroupMemberEntity {
		...
		return {
			...
			group: model.group()
				...
				: null,
			group_role: model.role()
				? this._groupRoleEntityMapper.toEntity(model.role())
				: null,
			user: model.user()
	...
	public toModel(entity: GroupMemberEntity): GroupMember {
		...
		return GroupMember.create(
			...
			entity.group_role
				? this._groupRoleEntityMapper.toModel(entity.group_role)
				: null,
			entity.is_creator,
