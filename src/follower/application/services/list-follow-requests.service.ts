...
export class ListFollowRequestsService implements ListFollowRequestsUseCase 
{
    ...
    public async execute(command: ListFollowRequestsCommand): Promise<Pagination<GetFollowRequestDto>>
    {
        const authenticatedUserId = this.getAuthenticatedUser.execute(null);

        if(command.from_user_id !== null && command.from_user_id !== undefined) {
            ...
            const isFollowing = (await this.followRepository.findBy({fromUserId: authenticatedUserId, toUserId: user.id()})).at(0) !== undefined;

            if(user.visibilityConfiguration().profileVisibility() === UserVisibility.FOLLOWERS && authenticatedUserId !== user.id() && !isFollowing) {
                return new Pagination([], 0, 0);
            }
    
            if(user.visibilityConfiguration().profileVisibility() === UserVisibility.PRIVATE && authenticatedUserId !== user.id()) {
                return new Pagination([], 0, 0);
            }
        }
        ...
        const items = pagination.items.map((i) => {
            return new GetFollowRequestDto(
                i.id(),
                {
                    ...
                    visibility_config: {
                        address_visibility: i.from().visibilityConfiguration().addressVisibility(),
                        poi_folder_visibility: i.from().visibilityConfiguration().poiFolderVisibility(),
                        post_visibility: i.from().visibilityConfiguration().postVisibility(),
                        profile_visibility: i.from().visibilityConfiguration().profileVisibility(),
                        visited_poi_visibility: i.from().visibilityConfiguration().visitedPoiVisibility()
                    }
                },
                {
                    id: i.to().id(),
                    ...
                    visibility_config: {
                        address_visibility: i.to().visibilityConfiguration().addressVisibility(),
                        poi_folder_visibility: i.to().visibilityConfiguration().poiFolderVisibility(),
                        post_visibility: i.to().visibilityConfiguration().postVisibility(),
                        profile_visibility: i.to().visibilityConfiguration().profileVisibility(),
                        visited_poi_visibility: i.to().visibilityConfiguration().visitedPoiVisibility()
                    }
                },
                i.requestedOn()
            );
