...
export class ListFollowsService implements ListFollowsUseCase 
{
    ...
    public async execute(command: ListFollowsCommand): Promise<Pagination<GetFollowDto>> 
    {
        ...
        if(command.from_user_id !== undefined && command.from_user_id !== null) {
            ...
            const isFollowingUser = (await this.followRepository.findBy({fromUserId: authenticatedUserId, toUserId: user.id()})).at(0) !== undefined;
            ...
            if(user.visibilityConfiguration().profileVisibility() === UserVisibility.FOLLOWERS && authenticatedUserId !== user.id() && !isFollowingUser) {
                return new Pagination([], 0, 0);
            }
        }
