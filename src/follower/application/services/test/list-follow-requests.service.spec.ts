...
describe('ListFollowRequestsService', () => {
    ...
    beforeAll(() => {
        const { unit, unitRef } = TestBed.create(ListFollowRequestsService).compile();
    ...
    it('should list follow requests', async () => {
        const user = mockUser();
        const command = new ListFollowRequestsCommand(
            user.id()
        );
        ...
        followRepository.paginateRequest.mockResolvedValue(new Pagination([mockFollowRequest(), mockFollowRequest(), mockFollowRequest()], 3, 0));
