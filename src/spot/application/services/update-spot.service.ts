...
export class UpdateSpotService implements UpdateSpotUseCase {
    ...
    public async execute(command: UpdateSpotCommand): Promise<void> {
        ...
        if (command.name !== null && command.name !== undefined && command.name.length > 0) {
            if (command.name !== spot.name() && await this.spotRepository.findByName(command.name) !== null) {
        ...
        if (command.description !== null && command.description !== undefined && command.description.length > 0) {
        ...
        if (command.address !== null && command.address !== undefined) {
            ...
            if (command.address.area !== null && command.address.area !== undefined && command.address.area.length > 0) {
                address.changeArea(command.address.area);
            }

            if (command.address.subArea !== null && command.address.subArea !== undefined && command.address.subArea.length > 0) {
                address.changeSubArea(command.address.subArea);
            }

            if (command.address.countryCode !== null && command.address.countryCode !== undefined && command.address.countryCode.length > 0) {
                address.changeCountryCode(command.address.countryCode);
            }

            if (command.address.locality !== null && command.address.locality !== undefined && command.address.locality.length > 0) {
                address.changeLocality(command.address.locality);
            }

            if ((command.address.latitude !== null && command.address.latitude !== undefined) &&
            (command.address.longitude !== null && command.address.longitude !== undefined)) {
