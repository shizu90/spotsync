import {
	SpotFolderItem as SpotFolderItemPrisma,
	SpotFolder as SpotFolderPrisma,
} from '@prisma/client';
...
export type SpotFolderEntity = SpotFolderPrisma & {
	creator?: UserEntity;
	spots?: (SpotFolderItemPrisma & { spot?: SpotEntity })[];
};

export class SpotFolderEntityMapper
	implements EntityMapper<SpotFolder, SpotFolderEntity>
{
		...
		return {
			...
			spots: model.items()
				? model.items().map((item) => ({
						...
						spot: item.spot()
							? this._spotEntityMapper.toEntity(item.spot())
							: null,
					}))
				: [],
		};
	...
	public toModel(entity: SpotFolderEntity): SpotFolder {
		...
		return SpotFolder.create(
			...
			entity.creator
				...
				: null,
			entity.spots
				? entity.spots.map((item) =>
						SpotFolderItem.create(
							item.spot
								? this._spotEntityMapper.toModel(item.spot)
								: null,
							item.order_number,
							item.added_at,
						),
