import { PostAttachment as PostAttachmentPrisma, Post as PostPrisma } from "@prisma/client";
...
export class PostEntityMapper implements EntityMapper<Post, PostEntity> {
    ...
    public toEntity(model: Post): PostEntity {
        ...
        return {
            ...
            attachments: model.attachments() ? model.attachments().map(attachment => ({
                id: attachment.id(),
            ...
            thread: model.thread() ? this._postThreadEntityMapper.toEntity(model.thread()) : null,
        };
    }
    ...
    public toModel(entity: PostEntity): Post {
        ...
        return Post.create(
            ...
            entity.attachments ? entity.attachments.map(attachment => PostAttachment.create(
                attachment.id,
