...
export class PostRepositoryImpl implements PostRepository 
{
        ...
        return Post.create(
            ...
            User.create(
                ...
                UserVisibilityConfig.create(
                    prisma_model.creator.id,
                    prisma_model.creator.visibility_configuration.profile_visibility,
                    prisma_model.creator.visibility_configuration.address_visibility,
                    prisma_model.creator.visibility_configuration.poi_folder_visibility,
                    prisma_model.creator.visibility_configuration.visited_poi_visibility,
                    prisma_model.creator.visibility_configuration.post_visibility
                ),
                prisma_model.creator.created_at,
                prisma_model.creator.updated_at,
                prisma_model.creator.is_deleted
            ),
            prisma_model.attachments.map((a) => this.mapPostAttachmentToDomain(a)),
            prisma_model.parent ? 
                this.mapPostToDomain(prisma_model.parent)
            : null,
            prisma_model.children_posts.map((children_post) => this.mapPostToDomain(children_post)),
            prisma_model.group ? 
                Group.create(
    ...
    public async paginate(params: PaginateParameters): Promise<Pagination<Post>> {
    ...
    public async findAttachmentById(id: string): Promise<PostAttachment> {
        const postAttachment = await this.prismaService.postAttachment.findFirst({
            where: {
                id: id
            }
        });
    ...
    public async update(model: Post): Promise<Post> {
        ...
        await this.prismaService.$queryRawUnsafe(`DELETE FROM post_attachments WHERE post_id = '${model.id()}'`);
