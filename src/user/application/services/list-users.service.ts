...
export class ListUsersService implements ListUsersUseCase 
{
    ...
    public async execute(command: ListUsersCommand): Promise<Pagination<GetUserProfileDto>> 
    {
        ...
        const items = await Promise.all(pagination.items.map(async (u) => {
            ...
            const totalFollowers = (await this.followRepository.findBy({toUserId: u.id()})).length;
            const totalFollowing = (await this.followRepository.findBy({fromUserId: u.id()})).length;
