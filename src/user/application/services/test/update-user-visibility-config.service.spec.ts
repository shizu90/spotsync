...
describe("UpdateUserVisibilityConfigService", () => {
    ...
    beforeAll(() => {
        const { unit, unitRef } = TestBed.create(UpdateUserVisibilityConfigService).compile();
    ...
    it('should update user visibility config', async () => {
        ...
        expect(user.visibilityConfiguration().profileVisibility()).toBe(command.profileVisibility);
        expect(user.visibilityConfiguration().poiFolderVisibility()).toBe(command.poiFolderVisibility);
    });
