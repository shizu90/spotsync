...
import {
	PostEntity,
	PostEntityMapper,
} from 'src/post/infrastructure/adapters/out/mappers/post-entity.mapper';
import {
	UserEntity,
	UserEntityMapper,
...
export class LikeEntityMapper implements EntityMapper<Like, LikeEntity> {
	...
	public toEntity(like: Like): LikeEntity {
		...
		return {
			...
			post_id:
				like.likableSubject() === LikableSubject.POST
					? like.likable()?.id()
					: null,
			comment_id:
				like.likableSubject() === LikableSubject.COMMENT
					? like.likable()?.id()
					: null,
			user_id: like.user() ? like.user().id() : null,
			comment: null,
			post:
				like.likableSubject() === LikableSubject.POST && like.likable()
					? this._postEntityMapper.toEntity(like.likable() as Post)
					: null,
			user: like.user()
				? this._userEntityMapper.toEntity(like.user())
				: null,
		};
	}
